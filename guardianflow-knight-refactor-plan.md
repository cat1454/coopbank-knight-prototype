...
```txt
feature/guardianflow-knight-integration
```
...
Nhánh `feature/guardianflow-knight-integration` đã tích hợp **GuardianFlow Decision Intelligence** vào KNIGHT.

Trọng tâm của nhánh này gồm:

- GuardianFlow chấm điểm rủi ro giao dịch.
...
```txt
GuardianFlow = quyết định rủi ro
...
- GuardianFlow và KNIGHT đang nằm chung tầng.
- Khó phân biệt đâu là logic chấm điểm rủi ro, đâu là logic hành động bảo vệ.
- Adapter chuyển điểm GuardianFlow sang KNIGHT chưa có vùng riêng rõ ràng.
...
src/domain/
  guardian-flow/
    scoring/
      guardianFlow.ts
      guardianFlow.test.ts
    explain/
      explainMapper.ts
    adapters/
      guardianToKnightRisk.ts
    types.ts
    index.ts
...
### 4.2. Lý do tách `guardian-flow`

GuardianFlow là phần **ra quyết định rủi ro**.
...
### 4.4. Adapter GuardianFlow sang KNIGHT

Nếu GuardianFlow dùng risk score `0-100`, còn KNIGHT policy dùng thang `0-1000`, thì phần chuyển đổi nên có file riêng:

```txt
src/domain/guardian-flow/adapters/guardianToKnightRisk.ts
```
...
Không nên chứa toàn bộ logic transfer hoặc GuardianFlow demo.
...
Hook này chỉ nên bridge UI dashboard với GuardianFlow decision:
...
## 6. Tách GuardianFlow demo thành feature riêng

### 6.1. Cấu trúc đề xuất

```txt
src/features/guardianflow-decision/
  ui/
    RiskMeter.tsx
    AgentConsole.tsx
    ScamChecklist.tsx
...
src/features/guardianflow-decision/ui/RiskMeter.module.css
...
- KnightGuardTab
- BottomNav
- GuardianFlow demo panel

Bước 4: Tách GuardianFlow UI
- RiskMeter
- AgentConsole
- ScamChecklist
- ActionCenter
...
- Không để GuardianFlow demo UI xuất hiện trong customer flow bình thường.
```
...
```txt
You are working on branch feature/guardianflow-knight-integration.

Goal:
Refactor the repository structure safely without changing KNIGHT or GuardianFlow behavior.

Important context:
- GuardianFlow Decision Intelligence is integrated into KNIGHT.
- GuardianFlow risk score is 0-100 and adapted to KNIGHT 0-1000.
...
3. Split BankDashboard.tsx into:
   ...
   - features/guardianflow-decision
...
```txt
1. BankDashboard.tsx
2. domain/guardian-flow và domain/knight
3. GuardianFlowPanel / RiskMeter / AgentConsole
...
3. Khi mở rộng GuardianFlow hoặc KNIGHT sẽ không làm rối toàn bộ UI.
```

Thông điệp kiến trúc có thể dùng khi trình bày:

```txt
GuardianFlow phân tích rủi ro
