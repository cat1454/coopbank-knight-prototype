...
import { evaluateThreatLensScenario, threatLensScenarios } from "../../../domain/threatLens";
import type {
...
import { checklistItems } from "./threatLensUi";

interface ThreatLensEvaluationState {
  scenario: ThreatLensScenario;
  decision: ThreatLensRiskDecision;
  agentUpdates: ThreatLensAgentResult[];
}

const CONSENT_KEY = "knight_threatlens_consent";
const LEVEL_KEY = "knight_threatlens_level";
...
export function useThreatLensScenario(latestDecision: ThreatLensRiskDecision | null) {
  ...
  // ThreatLens Control Level state: max, standard, min
  const [threatLensLevel, setThreatLensLevelState] = useState<"max" | "standard" | "min">(() => {
    if (typeof window === "undefined") return "standard";
    return (window.sessionStorage.getItem(LEVEL_KEY) as "max" | "standard" | "min") || "standard";
  });

  const [selectedScenario, setSelectedScenario] = useState<ThreatLensScenarioId>("low_risk");
  const [fakeLatencyMs, setFakeLatencyMs] = useState(0);
  const [detailedMode, setDetailedMode] = useState(true);
  const [evaluation, setEvaluation] = useState<ThreatLensEvaluationState | null>(null);
  const [agentUpdates, setAgentUpdates] = useState<ThreatLensAgentResult[]>([]);
  ...
  const selectedScenarioData = useMemo(() => {
    return threatLensScenarios.find((scenario) => scenario.id === selectedScenario) ?? threatLensScenarios[0];
  }, [selectedScenario]);

  const setThreatLensLevel = (level: "max" | "standard" | "min") => {
    window.sessionStorage.setItem(LEVEL_KEY, level);
    setThreatLensLevelState(level);
  ...
  const runScenario = async (scenarioId = selectedScenario) => {
    ...
    const result = await evaluateThreatLensScenario(scenarioId, {
      fakeLatencyMs,
...
export type ThreatLensScenarioViewModel = ReturnType<typeof useThreatLensScenario>;
