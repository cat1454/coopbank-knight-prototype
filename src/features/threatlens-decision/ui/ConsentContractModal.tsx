...
export function ConsentContractModal({ isOpen, threatLens, onClose }: ConsentContractModalProps) {
...
<div className="operator-call-overlay" style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px", margin: "12px 0" }}>
              <label style={{ display: "flex", gap: "8px", alignItems: "start", cursor: "pointer" }}>
                <input
                  type="checkbox"
                  checked={threatLens.consentBiometrics}
                  onChange={(e) => threatLens.setConsentBiometrics(e.target.checked)}
                  style={{ marginTop: "2px" }}
                />
                <div>
              ...
              <label style={{ display: "flex", gap: "8px", alignItems: "start", cursor: "pointer" }}>
                <input
                  type="checkbox"
                  checked={threatLens.consentBehavioral}
                  onChange={(e) => threatLens.setConsentBehavioral(e.target.checked)}
                  style={{ marginTop: "2px" }}
                />
                <div>
              ...
              <label style={{ display: "flex", gap: "8px", alignItems: "start", cursor: "pointer" }}>
                <input
                  type="checkbox"
                  checked={threatLens.consentLocation}
                  onChange={(e) => threatLens.setConsentLocation(e.target.checked)}
                  style={{ marginTop: "2px" }}
                />
                <div>
          ...
          <label style={{ display: "flex", gap: "8px", alignItems: "start", cursor: "pointer", fontSize: "11px", marginBottom: "12px" }}>
            <input
              type="checkbox"
              checked={threatLens.consentChecked}
              onChange={(e) => threatLens.setConsentChecked(e.target.checked)}
              style={{ marginTop: "1px" }}
            />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", width: "100%" }}>
            <button
              ...
              onClick={() => {
                onClose();
                threatLens.setConsentChecked(false);
              }}
            >
              Hủy bỏ
            </button>
            <button
              type="button"
              disabled={!threatLens.consentChecked}
              style={{
                background: threatLens.consentChecked ? "var(--color-success)" : "#cbd5e1",
                border: "none",
                borderRadius: "8px",
                color: "white",
                fontWeight: "bold",
                padding: "10px",
                cursor: threatLens.consentChecked ? "pointer" : "not-allowed",
                fontSize: "var(--text-xs)"
              }}
              onClick={() => {
                threatLens.grantConsent();
                onClose();
              }}
            >
              Đồng ý và Kích hoạt
