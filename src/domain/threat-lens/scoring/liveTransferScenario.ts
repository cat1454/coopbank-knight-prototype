...
import type { ThreatLensScenario, ThreatLensTransactionEvaluationInput } from "../types";
...
function normalizeThreatLensText(value: string) {
  return value
    .normalize("NFD")
...
function isTrustedTransferRecipient(input: ThreatLensTransactionEvaluationInput) {
  const normalizedName = normalizeThreatLensText(input.recipientName);
  return input.recipientAccount === "19038472910" || normalizedName.includes("nguyen van b");
}

function isKnownRiskRecipient(input: ThreatLensTransactionEvaluationInput) {
  const normalizedName = normalizeThreatLensText(input.recipientName);
  return (
...
function hasScamContent(input: ThreatLensTransactionEvaluationInput) {
