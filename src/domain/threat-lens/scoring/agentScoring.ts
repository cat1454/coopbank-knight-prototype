...
import type { ThreatLensAgentName, ThreatLensAgentResult, ThreatLensScenario } from "../types";
import { clampScore, hourFromTimestamp } from "./scoringUtils";
import type { TwinEnrichedContext } from "./twinContext";

function buildAgent(agentName: ThreatLensAgentName, score: number, signals: string[], reasoning: string): ThreatLensAgentResult {
  return {
    agentName,
...
function scoreTransaction(scenario: ThreatLensScenario) {
...
function scoreDevice(scenario: ThreatLensScenario) {
...
function scoreBehavioral(scenario: ThreatLensScenario, twin?: TwinEnrichedContext) {
  const { transaction, userProfile } = scenario;
  const signals: string[] = [];
  let score = 10;
...
function scoreBeneficiary(scenario: ThreatLensScenario, twin?: TwinEnrichedContext) {
...
function scoreScam(scenario: ThreatLensScenario) {
...
export async function runThreatLensAgents(
  ...
  const scorers: Array<(s: ThreatLensScenario, ctx?: TwinEnrichedContext) => ThreatLensAgentResult> = [
    scoreTransaction,
    scoreDevice,
