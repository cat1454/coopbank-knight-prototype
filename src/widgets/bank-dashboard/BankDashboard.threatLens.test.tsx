...
function renderDashboard(options: { threatLensDemoEnabled?: boolean } = {}) {
  ...
  render(
    <BankDashboard
      ...
      threatLensDemoEnabled={options.threatLensDemoEnabled}
    />,
  );
...
describe("BankDashboard ThreatLens Decision Intelligence", () => {
  afterEach(() => {
  ...
  it("keeps scenario controls available only in explicit demo mode", async () => {
    window.sessionStorage.setItem("knight_threatlens_consent", "withdrawn");
    const user = userEvent.setup();
    renderDashboard({ threatLensDemoEnabled: true });
